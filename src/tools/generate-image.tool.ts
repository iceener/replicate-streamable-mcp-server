...
export const GenerateImageInputSchema = z
  .object({
    model: z
      ...
      .regex(
        /^[^/]+\/[^/]+$/,
        'Model must be in format "owner/name" (e.g., "black-forest-labs/flux-schnell")',
      )
      .describe(
        'Full model identifier in format "owner/name" (e.g., "black-forest-labs/flux-schnell"). Use search_models if unsure.',
      ),
    input: z
...
Use search_models to see the exact requirements for "${model}".`,
    ...
    try {
      ...
      const prediction = await (context.services?.runPrediction ?? runPrediction)(
        model,
        input,
        context.replicateToken,
        context.signal,
      );
