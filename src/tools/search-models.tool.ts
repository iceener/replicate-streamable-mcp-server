...
export const SearchModelsInputSchema = z
  .object({
    query: z
      ...
      .describe(
        'Search query - model name, task type, or keywords (e.g., "flux", "image generation", "upscale")',
      ),
  })
  .strict();
...
export const searchModelsTool = {
  ...
  handler: async (args: unknown, context?: RequestContext): Promise<CallToolResult> => {
    ...
    try {
      ...
      const models = await (context.services?.searchModels ?? searchModels)(
        query,
